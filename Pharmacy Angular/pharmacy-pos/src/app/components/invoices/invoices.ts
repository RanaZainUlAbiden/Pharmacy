...
export class InvoicesComponent implements OnInit, OnDestroy {
  ...
  printInvoice() {
    ...
    // ============ HEADER SECTION (Dynamic from Settings) ============
    doc.setFontSize(14);
    ...
    for (const item of this.invoiceItems) {
      ...
      doc.text(`${item.price * item.quantity}`, pageWidth - 12, yPos);
      yPos += 5;
    }
    ...
    doc.text(`${this.selectedInvoice?.total_amount}`, rightX, yPos);
    yPos += 5;
    
    doc.text('Discount:', leftX, yPos);
    doc.text('0', rightX, yPos);
    yPos += 5;
    
    doc.text('Tax (17%):', leftX, yPos);
    const taxAmount = this.selectedInvoice?.total_amount * 0.17;
    doc.text(`${Math.round(taxAmount)}`, rightX, yPos);
    yPos += 5;
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.text(`Paid: ${this.selectedInvoice?.paid_amount}`, leftX, yPos);
    yPos += 4;
    doc.text(`Due: ${this.selectedInvoice?.due_amount}`, leftX, yPos);
    yPos += 8;
