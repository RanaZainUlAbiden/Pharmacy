...
export class ReorderComponent implements OnInit, OnDestroy {
  ...
  // ── Data ─────────────────────────────────────────────────────────────────────
  lowStockMedicines: any[] = [];  // Only low stock medicines with stock > 0
  reorderList: any[] = [];
 ...
 async loadData() {
  ...
  try {
    ...
    // ✅ UPDATED QUERY with default threshold
    const lowStock = await this.dbRun(`
      SELECT
        ...
        (COALESCE(NULLIF(m.minimum_threshold, 0), 10) - COALESCE(SUM(bi.quantity_remaining), 0)) AS needed_quantity
      FROM medicines m
      LEFT JOIN batch_items bi ON m.product_id = bi.product_id
      GROUP BY m.product_id
      HAVING current_stock <= minimum_threshold AND current_stock > 0
      ORDER BY needed_quantity DESC
    `);
    
    console.log('Found low stock medicines:', lowStock?.length);
  ...
  // Initialize reorder list with low stock items (stock > 0 and below threshold)
  initReorderList() {
    this.reorderList = this.lowStockMedicines.map((item: any) => ({
  ...
  // ── Dropdown — show only low stock medicines that are not already in list ─────

  get availableMedicines(): any[] {
    const existingIds = new Set(this.reorderList.map(i => i.product_id));
    return this.lowStockMedicines.filter(m => !existingIds.has(m.product_id));
  }
  ...
  addMedicineToReorder() {
    ...
    const med = this.lowStockMedicines.find(m => m.product_id === id);
    if (!med) return;
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: 'Courier New', monospace; width: 80mm; margin: 0 auto; padding: 4mm 2mm; font-size: 11px; line-height: 1.4; }
    .header { text-align: center; margin-bottom: 10px; padding-bottom: 8px; border-bottom: 1px solid #000; }
    .header h2 { font-size: 16px; font-weight: bold; margin: 0 0 4px; letter-spacing: 1px; }
    .header p { font-size: 9px; margin: 2px 0; }
    .title { text-align: center; margin: 10px 0; }
    .title p { font-size: 12px; font-weight: bold; }
    .info-row { display: flex; justify-content: space-between; margin: 5px 0; font-size: 9px; }
    .solid-line { border-top: 1px solid #000; margin: 8px 0; }
    table { width: 100%; border-collapse: collapse; margin: 8px 0; }
    th, td { padding: 6px 2px; text-align: left; border-bottom: 1px solid #ccc; }
    th { font-size: 10px; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px; }
    td { font-size: 10px; }
    td.center { text-align: center; }
    .sno { width: 15%; text-align: center; }
    .mname { width: 60%; word-break: break-word; white-space: normal; }
    .bold { font-weight: bold; }
    .total { display: flex; justify-content: space-between; margin-top: 10px; padding-top: 8px; border-top: 1px solid #000; font-weight: bold; font-size: 11px; }
    .footer { text-align: center; margin-top: 15px; padding-top: 8px; border-top: 1px solid #000; font-size: 8px; }
...
<body>
  ...
  <div class="info-row"><div>Bill No: ${serial}</div><div>Date: ${dateStr}</div></div>
  <div class="info-row"><div>Time: ${timeStr}</div><div>Items: ${this.reorderList.length}</div></div>
  <div class="solid-line"></div>
  <table>
    <thead><tr><th class="sno">#</th><th class="mname">Medicine Name</th><th class="center">Qty</th></tr></thead>
    <tbody>${rows}</tbody>
  </table>
  <div class="solid-line"></div>
  <div class="total"><span>TOTAL ITEMS</span><span>${this.totalQuantity}</span></div>
  <div class="solid-line"></div>
  <div class="footer"><p><strong>THANK YOU!</strong></p><p>Visit Again</p><p>GST: 12-345678-9</p></div>
